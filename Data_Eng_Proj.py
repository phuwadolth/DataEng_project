        ...
        if main_option == "ตรวจสอบและทำความสะอาดข้อมูลเบื้องต้น":
            ...
            if not numeric_cols:
                ...
                if not outliers.empty: # ถ้ามี outlierจะแสดงตัวอย่าง 50 แถวแรกในตาราง
                    ...
                    action = st.radio(
                        "ต้องการจัดการ Outlier อย่างไร?",
                        ("ลบทิ้ง", "แทนที่ด้วยค่ามัธยฐาน (median)"),
                        key="outlier_action"
                     )
                    #ปุ่มลบค่าOutlier
                    if st.button(f"🗑️ ลบ Outlier ในคอลัมน์ {box_col}"):
                        before = len(df) #จำนวนแถวก่อนลบ
                        df = df.loc[~mask_out].copy() #เลือกเฉพาะแถวที่ ไม่ใช่ outlier
                        removed = before - len(df) #จำนวนแถวที่ถูกลบ
                        st.success(f"✅ ลบ Outlier แล้ว {removed} แถว")
                        st.session_state.df = df.copy() #อัปเดต st.session_state.df เพื่อเก็บ DataFrame ใหม่
